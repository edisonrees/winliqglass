        ...
        if mode == 'studio':
            for gid, slots in (('tool', st['tool_slots']), ('util', st['util_slots'])):
                for sid, cx, cy in slots:
                    ...
                    if active:
                        dr.ellipse([cx - 19, cy - 19, cx + 19, cy + 19],
                                   fill=(255, 255, 255, 210))
                    elif hovered:
                        dr.ellipse([cx - 19, cy - 19, cx + 19, cy + 19],
                                   fill=(255, 255, 255, 110))
                    ICONS[sid](dr, cx, cy, 15, INK if (active or hovered) else (30, 32, 38, 190))
            for label, x0, x1, cy, v in st['sliders']:
                _text(dr, (x0 - 12, cy), label, self.f13, anchor='rm', fill=INK_DIM)
            # menus
            for m in st['menus']:
                _i_dots(dr, m['x'], m['y'], 13, INK)
                pop = m['pop']
                    ...
                    for i, (label, icon) in enumerate(items):
                        ...
                        if i == m.get('hover'):
                            dr.rounded_rectangle([px + 6, iy - ih / 2 + 3,
                                                  px + pw - 6, iy + ih / 2 - 3],
                                                 radius=13,
                                                 fill=(255, 255, 255, int(150 * fade)))
                        dr.text((px + 20, iy), label, font=self.f14,
                                fill=(30, 32, 38, ia), anchor='lm')
                        ICONS[icon](dr, px + pw - 26, iy, 11, (30, 32, 38, ia))
                        if i < len(items) - 1:
                            y = py + pad + ih * (i + 1)
                            dr.line([px + 16, y, px + pw - 16, y],
                                    fill=(30, 32, 38, da), width=1)
        else:
            lk = st['lock']
